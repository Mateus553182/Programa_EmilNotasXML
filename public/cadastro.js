const PACKAGE_OPTIONS = {
  ...
  profissional: {
    label: 'Profissional',
    companyLimit: 5,
    modeHint: 'Permite vincular até 5 empresas no cadastro inicial.',
  },
  corporativo: {
    label: 'Corporativo',
    companyLimit: null,
    modeHint: 'Permite vincular empresas ilimitadas no cadastro inicial.',
  },
};
...
function validateCompaniesStep() {
  ...
  if (Number.isFinite(companyLimit) && cards.length > companyLimit) {
    message.style.color = 'var(--danger)';
...
function updateCompanyListState() {
  ...
  const atLimit = Number.isFinite(companyLimit) && cards.length >= companyLimit;
...
function updatePackageSummary() {
  ...
  selectedPackageHint.textContent = Number.isFinite(selectedPackage.companyLimit)
    ? `Limite inicial de ${selectedPackage.companyLimit} empresa(s) vinculada(s) neste cadastro.`
    : 'Empresas ilimitadas vinculadas neste cadastro.';
  companiesPlanHint.textContent = Number.isFinite(selectedPackage.companyLimit)
    ? `${selectedPackage.companyLimit} empresa${selectedPackage.companyLimit > 1 ? 's' : ''}`
    : 'Empresas ilimitadas';
...
btnAdicionarEmpresa.addEventListener('click', () => {
  const { companyLimit } = getPackageConfig();
  if (Number.isFinite(companyLimit) && empresaList.querySelectorAll('.empresa-card').length >= companyLimit) return;
  createCompanyCard();
});
